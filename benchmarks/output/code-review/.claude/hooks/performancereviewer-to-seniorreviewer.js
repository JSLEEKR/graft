...
const INPUT = path.resolve('.graft/session/node_outputs/performancereviewer.json');
const OUTPUT = path.resolve('.graft/session/node_outputs/performancereviewer_to_seniorreviewer.json');
...
let result = {
  ...
  "complexity_concerns": data["complexity_concerns"]
};
...
fs.appendFileSync(TOKEN_LOG, `[${timestamp}] Edge PerformanceReviewer->SeniorReviewer | ${original}B -> ${transformed}B (${reduction}% reduction)\n`);
