import type { Diagnostic, Hover, Location, CompletionItem, DocumentSymbol, Range } from 'vscode-languageserver/node';
...
function makeSymbol(name: string, kind: SymbolKind, loc: SourceLocation): DocumentSymbol {
...
export function collectRenameLocations(docText: string, name: string): Range[] {
  ...
  // Build line offset index for fast line/character lookup
  const lines = docText.split('\n');
